...
class TestFlaskApiUsingRequests(unittest.TestCase):
    ...
    def test_delete_transaction(self):
        wallet_id = 1
        transaction_id = 20
        response = requests.delete('http://127.0.0.1:5000/wallet/'+str(wallet_id)+'/transaction/'+str(transaction_id))
        self.assertEqual(response.json()['message'], 'ok')
    def test_do_transaction(self):
        ...
        response = requests.post('http://127.0.0.1:5000/wallet/1/transaction',data=payload)
        self.assertEqual(response.json()['message'], 'ok')
